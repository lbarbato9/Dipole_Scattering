...
function [H_phi,H_pho,H_theta,E_phi,E_pho,E_theta] = wavefunc_2D_multimodal_dipoleTE(pol,n_tot,n_points,w,mu,k,pho,theta,phi,Ep,Er)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Loop coil is a TE polarization storage of m sum

% E-Field
Etheta_field_acc= Ep(n).*M3_theta + Er(n).*M4_theta; 
E_theta=E_theta +Etheta_field_acc;
 
Ephi_field_acc= Ep(n).*M3_phi + Er(n).*M4_phi; 
E_phi= E_phi + Ephi_field_acc;

% H-Field
Hphi_field_acc=(Ep(n).*N3_phi + Er(n).*N4_phi);
H_phi= H_phi + Hphi_field_acc;

Hpho_field_acc=(Ep(n).*N3_pho + Er(n).*N4_pho);
H_pho= H_pho + Hpho_field_acc;

Htheta_field_acc=(Ep(n).*N3_theta + Er(n).*N4_theta);
H_theta=H_theta + Htheta_field_acc;

end

H_theta=H_theta*(k/(1i*w*mu));
H_phi=H_phi*(k/(1i*w*mu));
H_pho=H_pho*(k/(1i*w*mu));
