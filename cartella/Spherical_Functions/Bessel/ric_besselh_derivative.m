...
% to calculate the first derivative of the Riccati-Bessel function.
%
%
% H_{nu}(x) = \sqrt{\frac{\pi x}{2}} H_{nu+1/2}(x)
%
% nu        order of the riccati-Hankel's function. Must be a
%           columne vector.
%
...
%
    ...
    if (K==1)
        H = ric_besselj_derivative(nu,x,flag) + 1j*ric_besselj_derivative(nu,x,flag);
    else
        H = ric_besselj_derivative(nu,x,flag) - 1j*ric_besselj_derivative(nu,x,flag);
    end
